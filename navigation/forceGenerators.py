...
class ForceGenerator:
    ...
    def _distToForceMag(self, dist: float) -> float:
        ...
        """
        dist = abs(dist)
        forceMag = logisticFunc(
            -1.0 * dist, self.strength, self.fieldSteepness, -self.radius
        )
...
class PointObstacle(ForceGenerator):
    ...
    def __init__(
        self, location: Translation2d, strength: float = 1.0, radius: float = 0.3
    ):
...
class LinearForceGenerator(ForceGenerator):
    ...
    def _shortestTransToSegment(self, point: Translation2d) -> Translation2d:
        ...
        t = (
            start_to_point_x * segment_vector_x + start_to_point_y * segment_vector_y
