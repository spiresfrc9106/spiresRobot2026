...
class MaxMotionController(MotorModuleController):
    ...
    def __init__(
        self, cals: MotorModuleCals, userUnitsToRadPerSec: Callable[[float], float]
    ) -> None:
        super().__init__(cals)
        self.oldTargetRadPerS: float = 0.0
        self.userUnitsToRadPerSec = userUnitsToRadPerSec
        self.kA_voltsPerRPMinSecs: float = field(
            init=False,
            default_factory=lambda: _secPerRadToMinPerRev(self.cals.kA.get()),
        )
    ...
    def updatePIDandFF(self, io: MotorModuleIO) -> None:
        ...
        io.setPIDFF(
            ...
            self.cals.kS.get(),
            kV_voltsPerRPM,
            self.kA_voltsPerRPMinSecs,
        )
        io.setMaxMotionVelParams(
