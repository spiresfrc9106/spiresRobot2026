...
class SwerveModuleControl:
    ...
    def __init__(
        ...
        self.moduleName, self.wheelMotor, self.azmthMotor, self.azmthEnc = (
    ...
    def setClosedLoopGains(self, gains: SwerveModuleGainSet):
        ...
        if (
            math.isclose(gains.wheelS.get(), 0)
            and math.isclose(gains.wheelV.get(), 0)
            and math.isclose(gains.wheelA.get(), 0)
        ):
            self.wheelMotorFF = None
        else:
            print(
                f"{self._speedDesTopicName} kS={gains.wheelS.get()} kV={gains.wheelV.get()} kA={gains.wheelA.get()}"
            )
    ...
    def update(self):
        ...
        if self.wheelMotorFF is None:
            ...
            motorVoltageFF = (
                self.kS * self.sign(motorDesSpd)
                + self.kV * motorDesSpd
                + self.kA * motorDesAccel
            )
